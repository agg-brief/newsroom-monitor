"""Streamlit Cloud newsroom dashboard with automatic RSS ingestion."""
...
def render_post(post: dict[str, object]) -> str:
    """Render a compact, fully clickable post row."""
    text = str(post["text"])
...
# Streamlit Cloud has no separate cron worker. Sync RSS in the app process so
# the online deployment works without a local computer or paid service.
now = time.time()
last_sync = float(st.session_state.get("last_rss_sync", 0.0))
if now - last_sync >= settings.refresh_seconds:
    try:
        entries = collect_rss(settings.rss_sources())
        inserted = database.save_many(entries)
        LOGGER.info("RSS sync: %d fetched, %d inserted", len(entries), inserted)
    except Exception:  # noqa: BLE001
        LOGGER.exception("RSS sync failed")
    st.session_state.last_rss_sync = now
...
.columns { display:grid; grid-template-columns:repeat(5,minmax(210px,1fr)); gap:.45rem; }
...
.post-list { height:calc(100vh - 155px); min-height:300px; overflow-y:auto; }
.post { display:grid; grid-template-columns:2.65rem 5.6rem minmax(0,1fr); align-items:center; gap:.3rem; min-height:28px; padding:.18rem .35rem; border-bottom:1px solid #202a33; color:#dce5ec; font-size:.73rem; line-height:1.15; text-decoration:none!important; }
...
.time { color:#8090a0; font-variant-numeric:tabular-nums; }.source { color:#61b4e6; font-weight:700; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }.headline { white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }.empty { color:#687788; font-size:.75rem; padding:.65rem .45rem; }
@media(max-width:1100px){.columns{grid-template-columns:repeat(2,minmax(250px,1fr));}.post-list{height:55vh;}} @media(max-width:650px){.columns{grid-template-columns:1fr;}.post-list{height:45vh;}}
</style>
""", unsafe_allow_html=True)

st.markdown("<h1>🛰️ Newsroom Monitor</h1>", unsafe_allow_html=True)
st.markdown(f'<div class="subtitle">Live RSS monitoring · refreshes every {settings.refresh_seconds}s · {datetime.now(timezone.utc).astimezone():%H:%M:%S}</div>', unsafe_allow_html=True)
